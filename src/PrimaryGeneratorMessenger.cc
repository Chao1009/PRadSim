...
PrimaryGeneratorMessenger::PrimaryGeneratorMessenger(PrimaryGeneratorAction *act) : G4UImessenger(), Action(act)
{
    ...
    EventTypeCmd->SetGuidance("  Choice : elastic, disintegration, moller, inelastic, trident");
    EventTypeCmd->SetParameterName("evtype", false);
    EventTypeCmd->SetCandidates("elastic disintegration moller inelastic trident");
